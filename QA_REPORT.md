...
- The user pasted the SVG of the uploaded teal-orbit logo made by an
  ...
  stored as `model_result/absolute_test_svg.svg` (replaced 2026-09-26 with
  the complete 43-path paste; audits identical: 97.0%, mae 0.64, ssim
  0.9862, IoU 0.9703, edge F1 0.9312). Strict audit measures it
  at **97.0%** (mae 0.66, ssim12 0.986, silhouette IoU 0.970, edge F1 0.93)
  — evidence: `qa/audits/absolute_test_svg/`.
- **`model_result/` folder (visible deliverable, browsable viewer):**
  `/model_result/index.html` (StaticFiles mount in app/main.py) shows all 50
  rows as input-vs-SVG cards with strict verdicts; regenerated by
  `scripts/model_result_gallery.py`. up to 50 fresh
